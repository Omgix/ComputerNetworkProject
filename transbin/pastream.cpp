...
const int LEN_CONTENT = 800;        // Number of bits in a packet
const int BYTES_CONTENT = LEN_CONTENT / 8;
const int LEN_CRC = 32;
const int BYTES_CRC = LEN_CRC / 8;
const int BITS_PACKET = LEN_CRC + LEN_CONTENT;
const int SIZE_PACKET = BITS_PACKET / 8;
...
void Stream::open_input_stream(void *data, PaStreamCallback callback)
{
	err =  Pa_OpenStream(
			&stream,
			&inputParameters,
...
void Stream::open_output_stream(void *data, PaStreamCallback callback)
{
	err = Pa_OpenStream(
		&stream,
		nullptr,
...
void Stream::send(SendData &data, bool writewaves, const char* file_name)
{
	...
	unsigned total = (data.size * BITS_PER_BYTE) +
		ceil((float)data.size * BITS_PER_BYTE / LEN_CONTENT) * BITS_PER_BYTE * BYTES_CRC;
	...
	if (err < 0) error();
	stop_stream();
	printf("###################### Sending has been done. #######################"); fflush(stdout);
	if (writewaves)
	{
				...
				void* userData)
{
	...
	if (data->status == SendData::sendingStatus::SIGNAL)
	{
		...
		if (index >= LEN_SIGNAL)
		{
			data->status = SendData::sendingStatus::CONTENT;
	...
	{
		unsigned long leftFrames = data->totalFrames - fileFrameIndex;
			...
			else
			{
				...
				*wptr++ = frame;
				if (index % SAMPLES_PER_BIT == 0)
					data->bitIndex++;
				if (index >= SAMPLES_PER_BIT * BITS_PACKET)
					data->prepare_for_new_sending();
			}
		}
...
SendData::SendData(const char* file_name, void *data_, SAMPLE *samples_) : 
	status(SIGNAL), fileFrameIndex(0), ext_data_ptr(data_ != nullptr), ext_sample_ptr(samples_ != nullptr),
	bitIndex(0), packetFrameIndex(0), isPreamble(true), isNewPacket(true),
	size(0), totalFrames(0), data((uint8_t*)data_), samples(samples_)
{
	...
	totalFrames = size * BITS_PER_BYTE * SAMPLES_PER_BIT + LEN_SIGNAL
		+ (LEN_PREAMBLE + LEN_CRC * SAMPLES_PER_BIT) * numPacket;
	if (!ext_sample_ptr)
...
ReceiveData::ReceiveData(unsigned max_time, void *data_, SAMPLE *samples_) :
	...
	frameIndex(INITIAL_INDEX_RX), startFrom(0)
{
	for (int i = 0; i < 5; ++i)
...
size_t ReceiveData::write_to_file(const char* path = nullptr)
{
	...
	unsigned num_bytes = (bitsReceived - ceil((float)bitsReceived / BITS_PACKET) * LEN_CRC) / BITS_PER_BYTE;
...
bool ReceiveData::demodulate()
{
	while (frameIndex + 2 < numSamplesReceived)
	{
		...
		else if (status == RECEIVING)
		{
			...
			frameIndex++;
			if (packetFrameIndex >= SAMPLES_PER_BIT * BITS_PACKET || 
				bitsReceived == 50000 + ceil((float)50000 / LEN_CONTENT) * LEN_CRC)
			{
				unsigned bitsReceivedPacket = packetFrameIndex / SAMPLES_PER_BIT;
				unsigned indexByte = (bitsReceived - bitsReceivedPacket) / BITS_PACKET * BYTES_CONTENT;
				...
				if (bitsReceived == 50000 + ceil((float)50000 / LEN_CONTENT) * LEN_CRC)
					return true;
				else
